...
<div id = "desktop">
    ...
    <h2>Podgląd zlecenia zwrotnego:<b> <?= $singleOrder[0]['client_name']; ?> <?= $singleOrder[0]['client_surname']; ?> / <?= $singleOrder[0]['laptop_producer']; ?> <?= $singleOrder[0]['laptop_model']; ?>  / <?= $singleOrder[0]['client_pickup_date']; ?> / <?= $singleOrder[0]['client_pickup_hours']; ?></b></h2>
        ...
        <tr>
            ...
            <td>
                <?php
                    ...
                    $notes = unserialize($singleOrder[0]['additional_notes']);
                    //printArrayHelper::printArray($notes);

                    if(!empty($notes[0]))
                    {
                        foreach($notes[0] as $k => $v)
                        {
                            if($k == $singleOrder[0]['shipment_id'])
                            {
                                echo '<span style="color:white; background: #9735be;">&nbsp;Przesyłka ' . $k . ': <i><b>' . $v . '&nbsp;</b></i></span></br>--</br>';
                            }
                            else
                            {
                                echo 'Przesyłka ' . $k . ': <i><b>' . $v . '</b></i></br>--</br>';
                            }
                        }
                    }
                    else
                    {
                        echo 'Brak notatek dla tego zlecenia.';
                    }
    ...
    <a href="?task=sendBackController&action=cancelCourierConfirm&order_id=<?= $singleOrder[0]['order_id']; ?>" class="w3-bar-item w3-button w3-orange">X Anuluj kuriera</a>
