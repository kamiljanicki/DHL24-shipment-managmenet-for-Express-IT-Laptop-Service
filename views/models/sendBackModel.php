...
class sendBackModel extends model
{
    ...
    public function cancelCourier($orderId)
    {
        ...
        if($restoreToArchive -> execute()) return TRUE;
    }
    ...
    public static function getBackTrackAndTraceInfo($shipmentId)
    {
        ...
        {
            $selfCall = new self;
            $selfCall -> moveToArchive($shipmentId, $trackingRaw);
        }
    ...
    private function moveToArchive($shipmentId, $trackingRaw)
    {
        ...
        $moveToArchive = $this -> dbDhlOrders -> query("UPDATE 
                                                                    orders 
                                                                    SET 
    ...
    public function forceArchive($orderId)
    {
        ...
        if($forceArchive -> execute()) return TRUE;


        if($this -> dbDhlOrders -> query("UPDATE orders SET 
                                                 shipment_status_history = '" . $shipmentHistory . "', 
                                                 additional_notes = '" . $additionalNotes . "',
                                                 current_status = '-2_archived'  WHERE order_id = " . $orderId)) return TRUE;
    }
}