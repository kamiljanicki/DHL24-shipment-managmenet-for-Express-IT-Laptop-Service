...
class trashModel extends model
{
    ...
    {
        $trashIndex = $this -> dbDhlOrders -> prepare("SELECT * FROM orders WHERE current_status = '-3_trashed'");
        $trashIndex -> execute();
    ...
    public function moveTo($post)
    {
        ...
        if($post['action_type'] == 'Waiting.')
        {
            ...
            if($moveToWaiting -> execute()) return TRUE;
        }
        ...
        if($post['action_type'] == 'New.')
        {
            ...
            if($moveToNew -> execute()) return TRUE;
        }
